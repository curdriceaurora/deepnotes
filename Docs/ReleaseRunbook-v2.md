...
**If tests fail:** do not proceed. Fix failures on a branch, merge to `main`, then restart from §1.
...
1. The signing certificate is valid and not expiring within the next 30 days. Check in **Keychain Access → My Certificates** or run:

```
security find-identity -v -p codesigning
```

2. The provisioning profile matches the target bundle ID and includes the correct entitlements. Check in **Xcode → Target → Signing & Capabilities** — there should be no warnings.

3. If either the certificate or profile is expiring soon, renew in the Apple Developer portal before proceeding. An expired identity mid-release means re-archiving and re-submitting.

### 2.3 Archive the build (Xcode app host)

In Xcode: **Product → Archive**. Validate the archive before uploading. The validation step will catch signing and entitlement problems before you attempt an upload.
...
### 2.4 Draft release notes

Write the App Store "What's New" text and update `CHANGELOG.md` (if maintained). Keep the notes user-facing — focus on what changed from the user's perspective, not internal refactors.

Save the "What's New" text somewhere you can paste it into App Store Connect during §3.2. A good default location is a comment in the git tag message or a dedicated section at the top of `CHANGELOG.md`.

### 2.5 Create the git tag
...
### 3.2 Beta soak period

Once the build clears Apple's review and is available on TestFlight:

1. Install on your primary device(s) and use the app normally for **at least 48 hours**. The goal is to catch issues that only surface with real-world usage — intermittent sync failures, slow migrations on large databases, battery drain, etc.
2. During the soak period, check Xcode Organizer daily for crash reports or energy diagnostics.
3. **Do not promote to production until the soak period has passed with no new issues.** If an issue surfaces, fix it, bump the build number, and restart from §1.

### 3.3 Production release (App Store)

Promote the approved TestFlight build to production in App Store Connect. Paste the "What's New" text drafted in §2.4. Set the release date or release manually.

---

## 4. Post-release verification

After the build is live, monitor for **72 hours** before considering the release stable.
...
- [ ] Check Xcode Organizer for crash reports. Any crash in the new build is a potential incident (→ §6).
- [ ] If the release uses a new migration, open the app on a device that was running the previous version and confirm the migration completes without errors.

### 24–72 hours

- [ ] Re-check Xcode Organizer. A crash-free rate below 99% on the new build warrants investigation.
- [ ] If you used phased rollout, confirm the rollout percentage is advancing on schedule and no new crash clusters have appeared.
- [ ] After 72 hours with no issues, the release is considered stable.
...
> **Known gap:** the "Reset Sync" UI does not exist yet. Until it ships, the only recovery path is reinstall, which loses any unsynced local data. If sync corruption can affect users at scale (e.g., a bad checkpoint value baked into a migration), treat it as a data-loss incident and follow §5.4 to halt distribution while preparing a patch. Prioritize shipping the Reset Sync action before any release that changes sync behavior.
